...
public interface HCNetSDK extends StdCallLibrary {
	...
	HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("C:\\haiKan\\lib\\HCNetSDK.dll", HCNetSDK.class);
	/*** 宏定义 ***/
	// 常量
...
//单用户参数(子结构)(扩展)
	public static class NET_DVR_USER_INFO_V40 extends Structure {
		...
		public byte byPriority; /* 优先级，0xff-无，0--低，1--中，2--高 */
		/*
		 * 无……表示不支持优先级的设置 低……默认权限:包括本地和远程回放,本地和远程查看日志和 状态,本地和远程关机/重启
		 * 中……包括本地和远程控制云台,本地和远程手动录像,本地和 远程回放,语音对讲和远程预览、本地备份,本地/远程关机/重启 高……管理员
		 */
...
//黑名单比对结果报警上传
	...
	public static interface FMSGCallBack extends StdCallCallback {
		public void invoke(NativeLong lCommand, NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen,
                           Pointer pUser);
	}
...
//前面板锁定
	...
	NativeLong NET_DVR_StartRemoteConfig(NativeLong lUserID, int dwCommand, Pointer lpInBuffer, int dwInBufferLen,
	...
	NativeLong NET_DVR_UploadFile_V40(NativeLong lUserID, int dwUploadType, Pointer lpInBuffer, int dwInBufferSize,
                                      String sFileName, Pointer lpOutBuffer, int dwOutBufferSize);
...
//gps返回数据结构定义
	...
	NativeLong NET_DVR_GetVehicleGpsInfo(NativeLong lUserID, NET_DVR_GET_GPS_DATA_PARAM lpGPSDataParam,
